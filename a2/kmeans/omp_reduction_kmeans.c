            ...
            float * clusters)         /* out: [numClusters][numCoords] */
{
    ...
    newClusterSize = (typeof(newClusterSize)) calloc(numClusters, sizeof(*newClusterSize)*sizeof(float));
    newClusters = (typeof(newClusters))  calloc(numClusters * numCoords, sizeof(*newClusters)*sizeof(float));

    //newClusterSize = (typeof(newClusterSize)) calloc(numClusters, sizeof(int));
    //newClusters = (typeof(newClusters)) calloc(numClusters * numCoords, sizeof(float));
    ...
    {
        local_newClusterSize[k] = (typeof(*local_newClusterSize)) calloc(numClusters, sizeof(**local_newClusterSize)*sizeof(float));
        local_newClusters[k] = (typeof(*local_newClusters)) calloc(numClusters * numCoords, sizeof(**local_newClusters)*sizeof(float));
    }
	...
	//}
        #pragma omp parallel for shared(numObjs, membership, local_newClusterSize, local_newClusters, numCoords, objects)  private(i, index, j) reduction(+:delta)
	for (i=0; i<numObjs; i++)
        {
