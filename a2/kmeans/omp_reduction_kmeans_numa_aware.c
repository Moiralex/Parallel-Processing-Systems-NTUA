            ...
            float * clusters)         /* out: [numClusters][numCoords] */
{
	...
	//{
         ...
         */
	#pragma omp parallel for shared(nthreads, numClusters, numCoords, local_newClusters, local_newClusterSize) private(k,i,j) 
	for (k=0; k<nthreads; ++k)
	{
		for(i=0; i<numClusters; ++i){
			for (j=0; j<numCoords; ++j)
	...
	//}
        ...
        //#pragma omp parallel for default(shared)  private(index,  j)
	for (i=0; i<numObjs; i++)
        {
