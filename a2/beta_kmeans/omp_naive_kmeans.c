            ...
            float * clusters)         /* out: [numClusters][numCoords] */
{
	...
	#pragma omp barrier
         ...
         */
	#pragma omp parallel for shared(newClusterSize, newClusters, membership, objects, clusters, numCoords, numObjs, numClusters) private(i, index, j) reduction(+:delta)
        for (i=0; i<numObjs; i++) {
	...
	#pragma omp barrier
        ...
        loop++;
        printf("\n  completed loop %d delta: %f", loop, delta);
        //fflush(stdout);
