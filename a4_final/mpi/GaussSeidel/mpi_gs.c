...
int main(int argc, char ** argv) {
	...
	*/
	...
	// sleep((grid[1]*rank_grid[0])+rank_grid[1]);
	...
	// barrier before starting computations
    ...
    for (t=0;t<T;t++) {
        ...
        // start communication for u_previous
        ...
        if (south_flag){
    		MPI_Isend(
				...
				MPI_COMM_WORLD, // community
				// &req
				&reqs_prev_sent[count_prev_neighbours]
			);
			// count_neighbours += 1;
			MPI_Irecv(
                ...
                0, // receive tag
                MPI_COMM_WORLD, // community
				&reqs_prev_received[count_prev_neighbours]
			);
			count_prev_neighbours+=1;
        }
		...
		MPI_Waitall(count_prev_neighbours, reqs_prev_received, MPI_STATUS_IGNORE);
        // end communication for u_previous

        swap=u_previous;
		u_previous=u_current;
		u_current=swap;

        // start receiving communication for u_current
        // if north or west neighbour exists, we need to receive current data before computation
        ...
        if (north_flag){
			MPI_Irecv(
                ...
                0, // receive tag
                MPI_COMM_WORLD, // community
				&reqs_curr_received[count_curr_neighbours]
			);
			count_curr_neighbours+=1;
        }
        // communicate with west neighbour
        if (west_flag)
        {
        ...
        MPI_Waitall(count_curr_neighbours, reqs_curr_received, MPI_STATUS_IGNORE);
        // end receiving communication for u_current
        ...
        // start sending communication for u_current
        count_curr_neighbours = 0;
        // if south and east neighbour exist, we need to send them current data
        // communicate with south neighbour
        if (south_flag){
    		MPI_Isend(
				(*u_current+local[0]*(local[1]+2)+1), // sendbuffer
				...
				MPI_COMM_WORLD, // community
				// &req
				&reqs_curr_sent[count_curr_neighbours]
			);
			count_curr_neighbours+=1;
        }
        // communicate with east neighbour
        ...
        // end sending communication for u_current
		...
		// collect for sent
        MPI_Waitall(count_curr_neighbours, reqs_curr_sent, MPI_STATUS_IGNORE);
        MPI_Waitall(count_prev_neighbours, reqs_prev_sent, MPI_STATUS_IGNORE);
		//************************************//
