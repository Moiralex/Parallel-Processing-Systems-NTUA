...
int main(int argc, char ** argv) {
	...
	*/
    ...
    for (t=0;t<T;t++) {
		...
		// testing
		// if (t % 10000 == 0) printf("Process rank[%d][%d] / iteration %d\n\n", rank_grid[0], rank_grid[1], t);
        ...
        if (south_flag){
            ...
            //     0, // receive tag
            //     MPI_COMM_WORLD, // community
			// 	&reqs_received[count_neighbours]
			// );
			// count_neighbours+=1;
        }
		...
		// MPI_Waitall(count_neighbours, reqs_received, MPI_STATUS_IGNORE);
		// MPI_Barrier(MPI_COMM_WORLD);
        // end communication
