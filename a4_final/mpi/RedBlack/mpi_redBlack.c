...
// copied from RedBlackSOR_serial
...
int main(int argc, char ** argv) {
	...
	*/
    ...
    for (t=0;t<T;t++) {
        ...
        // ------ COMPUTE RED based on previous timestep ------
        gettimeofday(&tcs,NULL);

		RedSOR(u_previous,u_current,i_min,i_max,j_min,j_max,omega);

		gettimeofday(&tcf,NULL);
        // ------ END COMPUTE RED based on previous timestep ------
        tcomp+=(tcf.tv_sec-tcs.tv_sec)+(tcf.tv_usec-tcs.tv_usec)*0.000001;


        // start communication for u_current
        ...
        // end communication for u_current

        // ------ COMPUTE BLACK based on current ------
        gettimeofday(&tcs,NULL);

		BlackSOR(u_previous,u_current,i_min,i_max,j_min,j_max,omega);

		gettimeofday(&tcf,NULL);
        // ------ END COMPUTE BLACK based on current ------
        tcomp+=(tcf.tv_sec-tcs.tv_sec)+(tcf.tv_usec-tcs.tv_usec)*0.000001;
