...
int main ( int argc, char ** argv ) {
	...
	struct timeval total_ts, total_tf;
	...
	gettimeofday(&total_tf,NULL);
	total_time+=(total_tf.tv_sec-total_ts.tv_sec)+(total_tf.tv_usec-total_ts.tv_usec)*0.000001;



	printf("Jacobi X %d Y %d Iter %d Time %lf Total_Time %lf midpoint %lf\n",X,Y,t-1,time,total_time, u_current[X/2][Y/2]);

	#ifdef PRINT_RESULTS
	char * s=malloc(30*sizeof(char));
	sprintf(s,"outdir/resJacobiNaive_%dx%d",X,Y);
